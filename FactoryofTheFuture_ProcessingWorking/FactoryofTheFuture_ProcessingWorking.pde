...
*/
 ...
 delay(100); // Set your desired interval here, in milliseconds
  ...
  //load info about WIND state - this doesn't work!!
 ...
 delay(100); // Set your desired interval here, in milliseconds
  ...
  //load info about WIND state - this is the one that works!! It replaced the grid one.
  String grid_state[] = loadStrings("http://localhost/website/windLEDstate.txt"); // Insert the location of your .txt file
  print(grid_state[0]);  // Prints whatever is in the file ("1" or "0" or "2")
 
  if (grid_state[0].equals("1") == true) {
    println(" - TELLING ARDUINO TO TURN GRID LED RED");
    port.write('L'); // Send "L" over serial to set RED GRID LED to HIGH
 
  } else if (grid_state[0].equals("0") == true){
